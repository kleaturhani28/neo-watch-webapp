...
begin
  FSessionGateway.Delete(Format('neowatch::session::%s', [Value.Id])).Value;
end;
...
begin
  ...
  if not Id.trim.IsEmpty then
    StringSession := FSessionGateway.Get(Format('neowatch::session::%s', [Id])).Value;
...
begin
  ...
  try
    FSessionGateway.Put(Format('neowatch::session::%s', [SessionDTO.Id]), SessionString).Value;
